...
### Aggressive Routing Mode (Technical Details)

**Problem it solves:**
Some Android OEMs (notably Samsung OneUI, certain Xiaomi/OPPO builds) implement custom audio routing layers that sit between the standard Android AudioManager APIs and the Bluetooth stack. These layers can override `AudioManager.setCommunicationDevice()` calls, especially at critical moments like when recording starts (`rec=true` edge). The standard 250 ms polling interval is too slow to detect and counter these overrides, causing the head unit microphone to be used instead of the interphone.

**How aggressive mode works:**

1. **Faster polling interval:** Reduces active session polling from 250 ms to 100 ms. This narrows the window during which an OEM override can occur without the app noticing.

2. **Immediate re-assert on recording edge:** When `AudioManager.isRecording()` transitions from `false` to `true` (the exact moment the user starts speaking), aggressive mode immediately calls `ForceSCO()` again, even if routing was already set. This targets the critical instant when OEM audio layers tend to override routing.

3. **Periodic re-assert even when device matches:** Every 3 timer ticks (~300 ms at 100 ms polling), aggressive mode re-calls `setCommunicationDevice()` even when `getCommunicationDevice()` already returns the correct target device. This pre-empts silent overrides that occur between polling intervals.

4. **Enhanced diagnostics:** When aggressive mode is active, the diagnostic log includes:
   - `[AGGRESSIVE] rec edge | before=XX:XX:XX:XX:XX:XX sco=true` - Device address and SCO state at the moment recording starts
   - `[AGGRESSIVE] Periodic re-assert result=true` - Whether the API accepted the re-assert call
   - `[AGGRESSIVE] Post-set verify addr=XX:XX:XX:XX:XX:XX match=true` - Verification that the system actually applied our routing choice
   - `[AGGRESSIVE] Session end | re-asserts=5` - Total number of re-asserts per session (high counts indicate constant conflict)

**Technical implementation:**

- **State tracking:** `prevRecording` boolean tracks the previous `isRecording` state to detect the `false→true` transition
- **Periodic counter:** `aggressiveReassertCounter` increments each tick, resetting to 0 when it reaches 3
- **Session counter:** `aggressiveSessionAsserts` accumulates total re-asserts per session for diagnostic summary

**When aggressive mode is enabled:**
- `timerMonitor_Tick` uses 100 ms interval instead of 250 ms during active sessions
- After `CheckAndForceSCO()` completes, checks for `rec=true` edge and immediately calls `ForceSCO()` again
- In `CheckAndForceSCO()`, when device already matches, periodically re-calls `setCommunicationDevice()` every 3 ticks
- In `ForceSCO()`, after successful `setCommunicationDevice()`, queries `getCommunicationDevice()` to verify the system actually applied the change

**Performance impact:**
- Battery: ~2.5x more timer ticks during active voice sessions (100 ms vs 250 ms), but only affects the duration of Assistant interactions, not idle time
- CPU: Minimal - just more frequent API calls to AudioManager
- Logging: Additional diagnostic entries prefixed with `[AGGRESSIVE]` when mode is active

**Diagnosis from logs:**
- `before=` shows wrong address at rec edge → System overrides at recording start; aggressive mode is targeting the right moment but may need even faster response
- `match=false` after ForceSCO → System silently ignored `setCommunicationDevice`; API-level problem, more aggressive timing won't help
- `result=false` on periodic re-assert → System is rejecting re-assert calls; may need alternative approach
- `sco=false` at rec edge → SCO itself is being disconnected, not just device switched; may need to also re-establish SCO
- High re-assert count (20+) per session → Constant fight with system; aggressive mode is working but system keeps overriding

**Design decision:**
Aggressive mode is **not tied to any device brand**. It is a user-facing option available on all phones. The user decides whether to enable it based on their experience. This avoids false assumptions about which OEMs have the routing-override problem and which do not. Some Samsung devices may work fine without it, while some non-Samsung devices may benefit from it.
