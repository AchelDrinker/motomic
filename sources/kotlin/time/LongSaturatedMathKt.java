...
public final class LongSaturatedMathKt {
    ...
    public static final long m1501saturatingAddNuflL3o(long j, DurationUnit unit, long j2) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        long jM1418toLongimpl = Duration.m1418toLongimpl(j2, unit);
        if (((j - 1) | 1) == Long.MAX_VALUE) {
            return m1500checkInfiniteSumDefinedPjuGub4(j, j2, jM1418toLongimpl);
        }
        if (((jM1418toLongimpl - 1) | 1) == Long.MAX_VALUE) {
            return m1502saturatingAddInHalvesNuflL3o(j, unit, j2);
        }
        long j3 = j + jM1418toLongimpl;
        return ((j ^ j3) & (jM1418toLongimpl ^ j3)) < 0 ? j < 0 ? Long.MIN_VALUE : Long.MAX_VALUE : j3;
    }

    /* JADX INFO: renamed from: checkInfiniteSumDefined-PjuGub4, reason: not valid java name */
    private static final long m1500checkInfiniteSumDefinedPjuGub4(long j, long j2, long j3) {
        if (!Duration.m1404isInfiniteimpl(j2) || (j ^ j3) >= 0) {
            return j;
        }
    ...
    private static final long m1502saturatingAddInHalvesNuflL3o(long j, DurationUnit durationUnit, long j2) {
        long jM1375divUwyO8pc = Duration.m1375divUwyO8pc(j2, 2);
        long jM1418toLongimpl = Duration.m1418toLongimpl(jM1375divUwyO8pc, durationUnit);
        return (1 | (jM1418toLongimpl - 1)) == Long.MAX_VALUE ? jM1418toLongimpl : m1501saturatingAddNuflL3o(m1501saturatingAddNuflL3o(j, durationUnit, jM1375divUwyO8pc), durationUnit, Duration.m1407minusLRDsOJo(j2, jM1375divUwyO8pc));
    }

    private static final long infinityOfSign(long j) {
        Duration.Companion companion = Duration.INSTANCE;
        return j < 0 ? companion.m1474getNEG_INFINITEUwyO8pc$kotlin_stdlib() : companion.m1473getINFINITEUwyO8pc();
    }

    public static final long saturatingDiff(long j, long j2, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        if ((1 | (j2 - 1)) == Long.MAX_VALUE) {
            return Duration.m1425unaryMinusUwyO8pc(infinityOfSign(j2));
        }
    ...
    private static final long saturatingFiniteDiff(long j, long j2, DurationUnit durationUnit) {
        ...
        if (((j3 ^ j) & ((j3 ^ j2) ^ (-1))) < 0) {
            if (durationUnit.compareTo(DurationUnit.MILLISECONDS) < 0) {
                ...
                Duration.Companion companion = Duration.INSTANCE;
                return Duration.m1408plusLRDsOJo(DurationKt.toDuration(j4, DurationUnit.MILLISECONDS), DurationKt.toDuration(j5, durationUnit));
            }
            return Duration.m1425unaryMinusUwyO8pc(infinityOfSign(j3));
        }
