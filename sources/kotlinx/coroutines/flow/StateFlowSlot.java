...
final class StateFlowSlot extends AbstractSharedFlowSlot<StateFlowImpl<?>> {
    ...
    public final void makePending() {
        ...
        while (true) {
            ...
            if (value == StateFlowKt.NONE) {
                if (AtomicReference$$ExternalSyntheticBackportWithForwarding0.m(this._state, value, StateFlowKt.PENDING)) {
                    return;
                }
            } else if (AtomicReference$$ExternalSyntheticBackportWithForwarding0.m(this._state, value, StateFlowKt.NONE)) {
                Result.Companion companion = Result.INSTANCE;
                ((CancellableContinuationImpl) value).resumeWith(Result.m45constructorimpl(Unit.INSTANCE));
                return;
            }
        }
    }
    ...
    public final Object awaitPending(Continuation<? super Unit> continuation) {
        ...
        if (!AtomicReference$$ExternalSyntheticBackportWithForwarding0.m(this._state, StateFlowKt.NONE, cancellableContinuationImpl2)) {
            if (DebugKt.getASSERTIONS_ENABLED() && Concurrent_commonKt.getValue(this._state) != StateFlowKt.PENDING) {
                throw new AssertionError();
            }
            Result.Companion companion = Result.INSTANCE;
            cancellableContinuationImpl2.resumeWith(Result.m45constructorimpl(Unit.INSTANCE));
        }
