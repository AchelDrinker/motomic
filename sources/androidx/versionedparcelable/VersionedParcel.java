...
public abstract class VersionedParcel {
    ...
    public Exception readException(Exception exc, int i) {
        int exceptionCode;
        return (readField(i) && (exceptionCode = readExceptionCode()) != 0) ? createException(exceptionCode, readString()) : exc;
    }
    ...
    public <K, V> Map<K, V> readMap(Map<K, V> map, int i) {
        ...
        EnumMap enumMap = (Map<K, V>) new ArrayMap(i2);
        if (i2 != 0) {
            ...
            for (int i3 = 0; i3 < i2; i3++) {
                enumMap.put(arrayList.get(i3), arrayList2.get(i3));
            }
        }
        return enumMap;
    }
